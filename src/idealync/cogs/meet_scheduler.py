...
class MeetSchedulerModal(ui.Modal, title="Schedule a meeting..."):
    ...
    description = ui.Label(text="Description", component=ui.TextInput(style=discord.TextStyle.paragraph))
    # discord only allows a max of 25 people in a user select
    user_select = ui.Label(text="Who's attending?", component=ui.UserSelect(max_values=25, min_values=2))
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if timestamp is None:
            await interaction.response.send_message("couldn't parse timestamp, did you spell something wrong?")
            return

        channel_id = await interaction.guild.fetch_channel(self.bot.config.meeting_voice_channel_id);

        if not isinstance(channel_id, discord.VoiceChannel):
            await interaction.response.send_message("MEETING_VOICE_CHANNEL_ID is not a voice channel.")
            return
        ...
        try:
            ...
            await interaction.response.send_message("bot doesn't have permissions to create an event")
            raise e
